...
export interface LocateBinaryOptions {
  ...
  /** Directory of this package (`apps/mcp-server`). */
  packageDir?: string;
  /** Extra roots to walk upward from (tests). */
  searchRoots?: string[];
}
...
export function locateMobileSimBinary(options: LocateBinaryOptions = {}): string {
  ...
  throw new Error(
    "Could not find the native `mobile-sim` binary. Set MOBILE_SIMULATOR_BIN, place `mobile-sim` next to this package, " +
      `or build it at ${RELEASE_RELATIVE} (from the repo root: npm run build:native).`,
  );
}
