...
/**
 * Line-delimited JSON-RPC over `mobile-sim rpc` stdio.
 * Viewer never talks to CoreSimulator.
 */
export class NativeRpcClient implements RpcClient {
  ...
  private constructor(child: ChildProcessWithoutNullStreams, defaultTimeoutMs: number) {
    ...
    child.on("exit", (code, signal) => {
      this.failAll(
        new Error(`mobile-sim rpc exited (code=${code ?? "null"}, signal=${signal ?? "null"}).`),
      );
    });
